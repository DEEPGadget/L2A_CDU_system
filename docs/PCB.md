...
| 주소 | 기능 | 설명 | 범위 |
|---|---|---|---|
| 0~3 | PWM Duty | **유량센서 급전** CH1~4 듀티비 (L2A Rev_C: T/G/V 채널, duty 100% 고정 → V=12 V. 변형별 매핑은 MCG.md §10.2 참고) | 0~1000 (0.0~100.0%) |
| 4~7 | PWM Duty | **팬** CH5~8 듀티비 | 0~1000 (0.0~100.0%) |
| 8~11 | PWM Duty | **펌프** CH9~12 듀티비 | 0~1000 (0.0~100.0%) |
| 12 | PWM Freq (TIM1) | CH1~4 주파수 (L2A Rev_C: 유량 급전 100% duty라 스위칭 없음 → 주파수 무관) | 1000~25000 Hz |
| 13 | PWM Freq (TIM2) | CH5~8 주파수 (L2A Rev_C: 팬, 운용 기본 **25000 Hz** — MCG 시작+재연결 시 write) | 1000~25000 Hz |
| 14 | PWM Freq (TIM8) | CH9~12 주파수 (L2A Rev_C: 펌프, 운용 기본 **1000 Hz** — MCG 시작+재연결 시 write) | 1000~25000 Hz |
...
>
> **센서 전원 = CH1~4 'V' 핀.** CH1~4 는 **T/G/V 채널**로 V = 12 V × PWM duty%. [`init_pcb_outputs()`](../src/mcg/main_loop.py) 가 **MCG 시작 + 재연결(poll 복구) 시** **CH1~4 duty=100% (HR 0~3 = 1000) → V = 12 V** 를 인가해 센서를 구동(VVX15 정격 12~24 V 의 하한). HR 0~3·HR 13/14 는 Flash 미저장이라 PCB 리셋 시 사라지므로 재인가 필요. 100% 에선 스위칭 없는 정전압이라 TIM1 주파수는 무관.
>
...
**MCS_IO 결선 (4 센서)**
- 각 VVX Pin 1 → CH1~4 'V' 핀(+12 V = duty 100%, fuse 권장), Pin 3 → CH1~4 'G'(PCB GND 공통)
- VVX Pin 4(PushPull) → CH1~4 'T' 펄스 입력:
  - **CH1 = IR 13**, **CH2 = IR 14** → **Loop1** (합산)
  - **CH3 = IR 15**, **CH4 = IR 16** → **Loop2** (합산)
- 단위 Hz. 환산: `branch_lpm = max(0, 0.12 × Hz)` (500 pulse/L), `loop_lpm = branch_a + branch_b`

**발행 Redis 키 (총합 + 4분기)**: 루프 총합 `sensor:flow_rate_1/2` + 분기별 `sensor:flow_rate_1_1`(CH1), `_1_2`(CH2), `_2_1`(CH3), `_2_2`(CH4). UI(Local 도식·Web 카드)는 **총합(큰글씨) + 분기 1/2(작은글씨)** 표시, History 는 `sensor_flow_rate_branch{loop,branch}` Prometheus 메트릭.
...
> ⚠ **펄스 입력 사양 / 분해능**
> 펄스 입력은 0~10 kHz (Falling Edge). VVX15 500 pulse/L 기준 40 L/min ≈ 333 Hz, 2 L/min ≈ 17 Hz → 입력 범위의 ~3 % 만 사용(여유 충분). PushPull high ≈ 12 V(= 'V' 공급) 스윙 — CH1~4 'T' 입력이 수용(실물 검증 완료).
> 참고: VVX15 측정범위 2~40 L/min, 무유량 시 lifesignal(설정 시 ~4 Hz) 이 켜져 있으면 ~0.5 L/min baseline 이 보일 수 있음.
...
> ⚠ **링크 끊김 시 거동 (현재)**: comm watchdog 미구현이라 MCG write 중단 시 보드는 **마지막 PWM duty 를 무한 유지**(0%/100% 아님). PDB 가 전력, 보드는 PWM 신호만 → 끊겨도 직전 설정대로 계속 구동. 안전 failsafe(무신호 시 자동 0/정의된 모드)는 **보드 펌웨어 watchdog 필요**.
> 🔌 **전원/링크 감지**: PDB→RPi 전용 power-good 신호선 **없음**. SW 상 보드 전원/링크 유일 지표 = **Modbus 응답 = `comm:status`**(MCG). RPi=5VSB 상시, 보드 12V=PDB. `comm:status=disconnected` 전환 시 MCG 가 폴링 센싱키(수온/유량/RPM/level/leak)를 **삭제 → UI no-data**(stale 방지). 명령 duty·ambient·control 은 유지.

| 기능 | 설명 | 비고 |
|---|---|---|
| Watchdog (Heartbeat 감시) | Master Heartbeat 미갱신 시 자동 모드 전환 | MCG 소프트웨어로 대체 불가 — PCB가 자체 판단해야 함. 현재: **링크 끊겨도 마지막 duty 유지** |
