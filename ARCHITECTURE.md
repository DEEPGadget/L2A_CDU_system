...
| **Flow 측정** | Rev_C+ 실 유량 센서 (루프당 2개 = 총 4개, SIKA VVX15 아날로그 전압 → IR 32~35, 루프별 2개 합산). 유량은 **실측값**이며 펌프 duty 추정이 아님. 센서 미가용 시 `sensor:flow_rate_*` 미발행 → UI no-data. [PCB.md "유량 추정"](docs/PCB.md) 참고 | 실물 설치 후 `_FLOW_SENSOR_ENABLED` 토글로 활성화 (Redis 키 / UI 무변경) |
