...
| **Flow 측정** | Rev_C+ 실 유량 센서 (루프당 2개 = 총 4개, SIKA VVX15 아날로그 0…10V → IR 32~35, 루프별 2개 합산, Q=4.0×V). 유량은 **실측값**이며 펌프 duty 추정이 아님. **Modbus 링크가 살아있으면 항상 측정**(별도 enable 플래그 없음), 센서/링크 미가용 시 `sensor:flow_rate_*` 미발행 → UI no-data. [PCB.md "유량"](docs/PCB.md) 참고 | (없음 — 링크 기반 자동) |
