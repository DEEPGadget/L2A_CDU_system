...
| **Flow 측정** | Rev_C+ 실 유량 센서 (루프당 1개 = 2개) 도입 진행 중 — `_read_flow_lpm()` hook 이 비어 있는 동안 MCG 가 펌프 duty 로부터 derive (`sensor:flow_rate_1/2`, 루프당 max 70 LPM). [PCB.md "유량 추정"](docs/PCB.md) 참고 | hook 채워지면 자동으로 실 센서 measurement 로 전환 (Redis 키 / UI 무변경) |
